...
final class CartHandler {
    ...
    init(container: DataSource.Container?) {
        ...
        _cancellable = actionInput
            ...
            .scan((Cart.empty, nil)) { currentCart, action -> CartResult in
                ...
                dispatchPrecondition(condition: .onQueue(DataSource.dbQueue))
                // }
    ...
    func trigger(action: CartAction) -> AnyPublisher<Void, Error> {
        let publisher = cartResult
            .subscribe(on: DataSource.dbQueue)
            .first()
            .tryMap { cartResult in
                // DLog("trigger recved:\n", cartResult.cart)
...
private func _perform(_ container: DataSource.Container?,
                      _ currentCart: Cart,
...
private func _dbAction(_ container: DataSource.Container?,
                       _ operation: (DataSource.WriteTransaction) -> Void = { _ in }) -> Error? {
    do {
        try container?.write {
