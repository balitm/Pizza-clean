...
public struct RepositoryUseCaseProvider: DatabaseContainerProtocol {
    private let _data: Initializer

    public init() {
        let container = Self.initContainer()
        _data = Initializer(container: container, network: RAPI.Network())
    }

    init(container: DataSource.Container? = nil, network: NetworkProtocol) {
